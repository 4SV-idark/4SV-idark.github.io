...
class ItcSubmitForm {
  ...
  constructor(target, config = {}) {
    this._attach = {
      ...
      maxFileSize: config['attachMaxFileSize'] || 2560, // максимальный размер файла
      ext: config['attachExt'] || ['pdf'], // дефолтные допустимые расширения для файлов
      items: []
    };
  ...
  _checkValidity() {
    ...
    if (elAttach) {
      ...
      if (isRequired && this._attach.items.length === 0) {
        ...
        elAttach.querySelector('.invalid-feedback').textContent = 'Прикрепите чертеж.';
      }
    }
